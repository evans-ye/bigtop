...
pushd .
cd $HADOOP_LIB_DIR
for d in $HADOOP_DEPENDENCIES; do
  CLASSPATH=$CLASSPATH:/usr/lib/hadoop/lib/$d
done
popd
sudo -u hdfs /usr/lib/bigtop-groovy/bin/groovy -classpath $CLASSPATH /usr/lib/bigtop-utils/init-hcfs.groovy /usr/lib/hadoop/libexec/init-hcfs.json
